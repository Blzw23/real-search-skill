...
它也会尽量避免另一种常见问题：资料看起来不少，但其实都挤在同一类来源里。严肃调研应该同时看到产品、源码、论文、社区和反例，而不是只看官网或只看一个项目。
...
5. 对论文、项目和源码分别写中文笔记，记录证据等级、PDF/源码阅读状态和不确定项。
6. 阶段性综合，形成对比矩阵、调研报告、学习路线或落地方案，并用质量门禁和 reviewer 清单复核浅度风险、资料偏科和过早收束。
...
根目录只放最后给人看的交付物，`工作材料/` 统一放研究计划、笔记、资料索引、自动发现、源码/PDF 和 reviewer 复核材料。不同任务会有不同产物。学习型任务更偏学习路线、概念地图、论文笔记；落地型任务更偏对比矩阵、机会分析、项目方案和 MVP 路线图。
...
| 自动发现资料 | 根据主题生成候选来源，包含论文库、GitHub、官方/产品/benchmark 入口，以及无登录社区讨论入口 |
| 处理论文 | 记录论文元数据、阅读队列、PDF 基础检查、正文抽取质量和笔记草稿 |
...
| 质量检查 | 检查资料索引、证据等级、源码阅读、论文队列、内容浅度风险，以及“是不是停在第一轮收束”的 warning |

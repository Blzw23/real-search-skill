...
- Root final deliverables: `调研报告.md` / `对比矩阵.md` / `落地方案.md` / `MVP路线图.md`
- `工作材料/研究计划.md`
- `工作材料/资料索引.md`
- `工作材料/阶段日志.md`
- `工作材料/论文阅读队列.md`
...
2. **Search Broadly**
   ...
   - Track breadth explicitly. A serious first pass should try to cover official/product, open-source/code, papers/technical reports, community/forum signals, and at least one counterexample or route that likely does not fit.
...
3. **Select Deep Sources**
   ...
   - Do not stop after the first converged summary if the evidence base is still thin. A serious run should usually include a second pass: deeper source reading after the first summary reveals where the evidence is weak.
...
- Root-level human deliverables
- A `工作材料/` folder for all model-facing process files
- A plan
- A source index
- Stage logs
- Per-source notes
...
- The breadth warnings in `check_quality.py` should be taken seriously. If the run is dominated by product pages, a single repo, or abstract-level paper notes, treat the synthesis as a first-pass draft rather than a finished report.
- `check_quality.py` should not leave `depth-has-multiple-deep-sources`, `depth-paper-notes-have-fulltext-pass`, or `depth-stage-log-has-second-pass` unresolved unless you clearly tell the user the research is still mid-run.

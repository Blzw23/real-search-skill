...
SYNTHESIS_FILES = ["调研报告.md", "学习总结.md", "落地方案.md", "MVP路线图.md", "对比矩阵.md"]
...
def main() -> None:
    ...
    if index_path.exists():
        ...
        checks.append({
            "check": "source-index-has-entries",
            "passed": len(data_rows) > 1,
            "evidence": f"{len(data_rows) - 1} source rows in {index_path}",
        })
    ...
    checks.append({
        "check": "has-per-source-notes",
        "passed": note_count > 0,
        "evidence": f"{note_count} markdown notes across note directories",
    })

    synthesis = [name for name in SYNTHESIS_FILES if (workspace / name).exists() and (workspace / name).stat().st_size > 20]
    checks.append({
        "check": "has-synthesis-document",
        "passed": bool(synthesis),
        "evidence": ", ".join(synthesis) if synthesis else "no non-empty synthesis document found",
    })
